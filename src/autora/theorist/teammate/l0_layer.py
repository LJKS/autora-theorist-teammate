...
class LZeroGate(nn.Module):
    ...
    def hard_concrete_cdf(self, s_dash):
    ...
    def regularization_loss(self):
        if LOSS_MODE == "binary_weighted":
            p_leq_0 = self.hard_concrete_cdf(torch.tensor(0., device=self.log_alpha.device))
            p_geq_1 = 1 - self.hard_concrete_cdf(torch.tensor(1., device=self.log_alpha.device))
            ...
            p_geq_1 = torch.mean(1 - self.hard_concrete_cdf(torch.tensor(1., device=self.log_alpha.device)))
            geq_1_loss = LAGRANGIAN_P_1_TARGET - p_geq_1
            p_leq_0 = torch.mean(self.hard_concrete_cdf(torch.tensor(0., device=self.log_alpha.device)))
